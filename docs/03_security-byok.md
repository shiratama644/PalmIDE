<!-- 元ファイル: Local_AI_Agent.md §3.1 Lines 89-132 -->
...
> **関連ファイル**: [02_architecture.md](./02_architecture.md) | [04_storage-persist.md](./04_storage-persist.md)（§3.2） | [05_sandbox.md](./05_sandbox.md)（ゼロトラスト） | [12_db-schema.md](./12_db-schema.md)（securityストア） | [README.md](../README.md)
> **元セクション**: §3.1（Lines 89-132）

> **要約**: 本ファイルはAES-GCM-256 / PBKDF2 (600,000 iterations, SHA-256, 256-bit Salt) によるAPIキー暗号化、2つの暗号化モード（暗号化保存/セッション限定）、セッション破棄（5分無操作でnull化）を定義する。ファイル永続化の詳細は 04_storage-persist.md を参照。
...
> 🆕 **詳細化補足（Phase 4）**
> - **対象**: `encryptApiKey` に対応する `decryptApiKey` 未定義、誤パスワード時の挙動未定義（§3.1）
> - **種別**: 🔴Blocker解消
> - **内容**: 以下の復号関数とエラー handling の叩き台を提案する。
>   ```typescript
...
>   ```
>   UIでは「パスワード再入力モーダル」を出し、3回失敗で「暗号化保存モードをリセットしますか？」の導線を出すことを推奨。5分無操作破棄後はメモリ上の鍵を `null` 化し、再入力を求める既存仕様と整合。
> - **根拠**: Web Crypto API の `decrypt` は `OperationError` で失敗を通知するのが標準。`encrypt` と対称な導出パラメータ（600k/SHA-256/256-bit Salt）を使えば round-trip が保証されるため。
> - **要検証**: `NIST SP 800-132` 準拠の `salt` 長は維持し、将来 `salt` を更新する際のマイグレーションは `12_db-schema.md` で扱う。

---

> **出典**: `Local_AI_Agent.md` §3.1（Lines 89-132）を一字一句維持して分割
